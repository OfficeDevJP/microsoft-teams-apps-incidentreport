            ...
            string onCallSMEUsers = "")
        {
            ...
            if (searchServiceResults != null)
            {
                foreach (var ticket in searchServiceResults)
                {
                    ...
                    {
                        Title = $"<b>{HttpUtility.HtmlEncode(ticket.Title)} | {HttpUtility.HtmlEncode(ticket.Severity == (int)TicketSeverity.緊急 ? localizer.GetString("緊急") : localizer.GetString("NormalText"))}</b>",
